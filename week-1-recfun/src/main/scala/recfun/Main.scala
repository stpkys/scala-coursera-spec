...
//    return sumf
   ...
   */
    def balance(chars: List[Char]): Boolean = {
      def test(current: List[Char], count: Int) : Boolean = {
        if (current.isEmpty) return count == 0
        if (count < 0) return false
        if (current.head == '(') return test(current.tail, count + 1)
        if (current.head == ')') {
          return test(current.tail, count - 1)
        }
        test(current.tail, count)
      }
   ...
   */
    def countChange(money: Int, coins: List[Int]): Int = {
      ...
      if (money == coins.head) return 1 + countChange(money, coins.tail)
