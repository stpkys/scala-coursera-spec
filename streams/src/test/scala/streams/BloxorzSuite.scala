...
class BloxorzSuite extends FunSuite {
  ...
  test("neighbours with history for level 1") {
    new Level1 {
      val res: Set[(Block, List[Move])] = neighborsWithHistory(Block(Pos(1, 1), Pos(1, 1)), List(Left, Up)).toSet
  ...
  test("new neighbours only") {
    new Level1 {
      val newNeighbours: Stream[(Block, List[Move])] = newNeighborsOnly(
        Set(
          (Block(Pos(1, 2), Pos(1, 3)), List(Right, Left, Up)),
          (Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))
        ).toStream,
      ...
      assert(Set((Block(Pos(2, 1), Pos(3, 1)), List(Down, Left, Up))) === newNeighbours.toSet)
    }
  }

  test("infinite terrain") {
    new InfiniteTerrain with Solver {
      /**
        * The target position where the block has to go.
        * This value is left abstract.
        */
      override val goal: Pos = Pos(0, 3)
      /**
